...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Bitm-m/Mask-xui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        wget -N --no-check-certificate https://raw.githubusercontent.com/Bitm-m/Mask-xui/main/x-ui.sh -O /usr/bin/x-ui
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/Bitm-m/Mask-xui/releases/download/v2.3.4/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
